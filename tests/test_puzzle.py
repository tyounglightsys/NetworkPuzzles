...
class TestCreateItems(unittest.TestCase):
    ...
    def test_createlink_good(self):
        prev_ct = len([i for i in session.puzzle.links])
        result = session.puzzle.createLink(["pc0", "net_switch0"])
        ct = len([i for i in session.puzzle.links])
...
class TestListItemsJson(unittest.TestCase):
    ...
    def test_alldevices(self):
        ...
        self.assertEqual(len(devices), len([d for d in session.puzzle.devices]))

    def test_alllinks(self):
        links = self.network.get("link")
        self.assertEqual(len(links), len([i for i in session.puzzle.links]))
...
class TestFilterItems(unittest.TestCase):
    def setUp(self):
        self.puzzles = [
            {"EduNetworkBuilder": {"Network": {"name": "puzzle1_dhcp"}}},
            {"EduNetworkBuilder": {"Network": {"name": "puzzle2_vlan"}}},
        ]
        self.puzzle_names = [
            "puzzle1_dhcp",
            "puzzle2_vlan",
        ...
        self.puzzle_files = [
            str(f).replace(".json", "") for f in PUZZLES_DIR.glob("*.json")
        ]
    ...
    def test_disk_filter(self):
        ...
        self.assertEqual(
            dhcp_files,
            puzzle.filter_items(self.puzzle_files, ".*DHCP.*", json_files=True),
        )

    def test_dist_nofilter(self):
        self.assertEqual(
            sorted(self.puzzle_files),
            sorted(puzzle.filter_items(self.puzzle_files, None, json_files=True)),
        )

    def test_memory_filter(self):
        self.assertEqual(
            self.puzzle_names[:1], puzzle.filter_items(self.puzzles, ".*DHCP.*")
        )
...
class TestXFromY(unittest.TestCase):
    ...
    def test_linkfromdevices_found(self):
        self.assertEqual(
            ...
            session.puzzle.link_from_devices(self.device1, self.device4).get(
                "uniqueidentifier"
            ),
        )
    ...
    def test_linkfromuid_found(self):
        self.assertEqual(self.link0, session.puzzle.link_from_uid("146"))
    ...
    def test_linkfromname_found(self):
        self.assertEqual(
            self.link0, session.puzzle.link_from_name("net_switch0_link_router0")
        )
