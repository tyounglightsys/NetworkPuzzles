...
"""Show contents of a cProfile data file.

The data file can be created with:
`python -m cProfile -o data.prof src/main.py`
"""
...
class Stats(pstats.Stats):
    ...
    # Ref: https://stackoverflow.com/a/78404643
    ...
    def sort_stats(self, *field):
      ...
      if len(field) == 1 and isinstance(field[0], int):
        ...
        for arg in field[1:]:
          if type(arg) is not type(field[0]):
...
def main():
    ...
    sortkey = 'cumulativepercall'
    if len(sys.argv) > 2:
      sortkey = sys.argv[2]
      # Assume 2nd arg is sort column

    try:
      stats.sort_stats(sortkey)
    except KeyError:
      print(f"Bad sort key. Valid options: {', '.join(Stats.sort_arg_dict_default.keys())}")
      sys.exit(1)
    stats.print_stats('/network_puzzles/gui', 20)
