...
class Device(ItemBase):
    ...
    def get_route_for_dest_ip(self, dest_ip: str):
        ...
        # A route must contain 'nic' and 'interface', and may contain 'gateway'.
        new_route = Route()
        ...
        for route_data in self.routes:
            ...
            if destinationIP in staticroute:
                ...
                logging.debug(f"Found a static route.  Adding gateway: {route.gateway}")
                new_route.gateway = route.gateway
                break

        # if not a device route, look through device nics
        if not new_route.gateway:
            ...
            for nic in self.nics:
                ...
                if dest_ip == "255.255.255.255":
                    skipzeroes = False
                localInterface = nic.find_local_interface(dest_ip, skipzeroes)
        ...
        if not new_route.interface and not new_route.gateway:
            ...
            logging.debug(f"Still no idea. Using default gateway: {self.gateway}")
            new_route.gateway = self.gateway
    ...
    def route_add(self, target, gateway):
        ...
        route = Route(
            ip=str(target_IP.ip), netmask=str(target_IP.netmask), gateway=gateway
        )
        ...
        mark_test_as_completed(
            ...
            route.ip,
            "NeedsRouteToNet",
            f"{self.hostname} successfully created route to {route.ip}",
        )
        ...
        mark_test_as_completed(
            ...
            route.network,
            "NeedsRouteToNet",
            f"{self.hostname} successfully created route to {route.network}",
        )
    ...
    def route_del(self, target, gateway):
        logging.debug(f"Deleting route {target} {gateway}")
        # if we have a route with the same src/dest/target, we drop it
        # if not, we return "false"
        try:
            target_IP = ipaddress.ip_interface(target)
    ...
    def ReturnIPConnectionEntry(self, dest_ip, src_ip, packettype):
        ...
        for onerec in self.ip_connections[:]:
            ...
            logging.debug(
                f"checking for response {onerec['dest']}, {onerec['src']} {onerec['packettype']}"
            )
            if (
                onerec["dest"] == packet.justIP(dest_ip)
                and onerec["src"] == packet.justIP(src_ip)
    ...
    def process_tunneled_packet(self, pkt):
        ...
        pkt.payload = (
            None  # remove the packet so it is not killed when the tunnel is killed
        )
        pkt.status = "done"  # Regardless, the packet ends here.
        logging.debug("----------")
        logging.debug(
            f"Unpacking a tunneled packet at {self.hostname} - {packetpayload.packettype} {packetpayload.source_ip} {packetpayload.destination_ip}"
        )
        logging.debug("----------")
        ...
        if self.routes_packets or deviceHasIP(self.json, pkt.destination_ip):
            ...
            if pkt.ttl <= 0:
                if pkt.packettype == "traceroute-request":
                    ...
                    logging.debug(
                        f"Packet TTL reached 0. killing packet at device: {self.hostname}"
                    )
                    logging.debug(
                        f"  Packet: {pkt.packettype} {pkt.source_ip} -> {pkt.destination_ip}"
                    )
                    pkt.status = "done"
                    return True
        ...
        if not packet.isEmpty(pkt.destination_ip) and deviceHasIP(
            ...
            # ping response, mark it as done
                ...
                if pingdest is not None and pkt.health == 100:
                    ...
                    session.puzzle.RegisterPingTestSuccess(
                        self.hostname, pingdest.get("hostname")
                    )
    ...
    def send_packet(self, pkt, inbound_nic=None, nic_out=None):
        ...
        route = self.get_route_for_dest_ip(pkt.destination_ip)
        if route is None:
        ...
        # Define route-dependent objects.
        route_interface = Interface(route.interface)
        route_nic = Nic(self.nic_from_name(route_interface.nicname))
    ...
    def _send_out_hubswitch(self, pkt, inbound_nic=None):
        ...
        for onenic in self.nics_data:
            ...
            # logging.debug(f"#### device type is {self.mytype} nic {n.type}")
            if device_nic.type not in ("port", "wport") and self.mytype not in (
                "wap",
                "wbridge",
                "wrepeater",
            ):
            ...
            if connected_link is not None and (
                ...
                if onlyport == "" or onlyport == device_nic.name:
                    ...
                    tpacket.distance = 0
                    # Set direction.
                    if connected_link["SrcNic"]["hostname"] == self.hostname:
